...
<div class="content-wrapper">
    ...
    <?php
        if (isset($_SESSION['errorgr'])) {
            ...
            ";
            unset($_SESSION['errorgr']);
        }
        if (isset($_SESSION['successgr'])) {
            echo "
            ...
            ";
            unset($_SESSION['successgr']);
        }
        if (isset($_SESSION['editgr'])) {
    echo "
                ...
                <div class="box">
                    ...
                    <a href="#" id="openAddModal" class="btn btn-primary btn-sm btn-flat" data-bs-toggle="modal" data-bs-target="#exampleModaladd">
                        ...
                        <div class="modal-dialog" style="max-width: 600px;">
                            ...
                            <div class="modal-body">
                            <form id="registrationForm" action="genre_add.php" method="POST" class="form-inline">
                                <div class="mb-3">
                ...
                <div class="modal fade" id="exampleModalEdit" tabindex="-1" aria-labelledby="exampleModalLabeledit" aria-hidden="true">
                    <div class="modal-dialog" style="max-width: 600px;">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="updateForm" class="form-inline" action="genre_edit.php" method="POST">
                                    <div class="mb-3" hidden>
                                        <label for="genreId" class="form-label">Genre ID</label>
                                        <input type="text" class="form-control" id="genreId" name="genre_id" readonly>
                                    </div>
                                    <div class="mb-3">
                                        <label for="genreName" class="form-label">Genre Name</label>
                                        <input type="text" class="form-control" id="genreName" name="nama_genre" required>
                                    </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-default btn-flat pull-left" data-bs-dismiss="modal"><i class="fa fa-close"></i> Close</button>
...
$no = $offset + 1;
$sql = mysqli_query($koneksi, "SELECT * FROM genre LIMIT $offset, $records_per_page");

$total_records_query = mysqli_query($koneksi, "SELECT COUNT(*) FROM genre");
...
if ($total_records > 0) {
    while ($data = mysqli_fetch_assoc($sql)) {
        echo "<tr>
                <td scope='row'>" . $no++ . "</td>
                <td class='genre-name'>" . $data['nama_genre'] . "</td>
                <td>
                    <button class='btn btn-success btn-sm edit btn-flat' data-id='" . $data['id_genre'] . "' data-bs-toggle='modal' data-bs-target='#exampleModalEdit'><i class='fa fa-edit'></i> Edit</button>
                    <a href='genre_delete.php?action=delete&id=" . $data['id_genre'] . "' class='btn btn-danger btn-sm delete btn-flat' onclick='return confirm(\"Are you sure you want to delete this student?\")'><i class='fa fa-trash'></i> Delete</a>
                </td>
            </tr>";
    }
} else {
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.edit').click(function() {
            var genreId = $(this).data('id'); // Get the genre ID from data attribute
            var genreName = $(this).closest('tr').find('.genre-name').text(); // Get the genre name from the specific cell
            $('#genreId').val(genreId); // Set the genre ID in the modal
            $('#genreName').val(genreName); // Set the genre name in the modal
        });
    });
</script>
